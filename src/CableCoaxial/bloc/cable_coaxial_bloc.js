...
export default class CableCoaxialBloc {
    ...
    calcularImpedancia(cableCoaxial, f, ur, er, s, tand){
        ...
        if(f < 3*this.bigmath.pow(10,6)){
            ...
            let y = this.bigmath.pow(cableCoaxial.b,2) - 3*this.bigmath.pow(cableCoaxial.c,2) + ((4*this.bigmath.pow(cableCoaxial.c,4))/(this.bigmath.pow(cableCoaxial.c,2)-this.bigmath.pow(cableCoaxial.b,2)))*this.bigmath.log(cableCoaxial.c/cableCoaxial.b)
        
            let L = (u/(2*this.bigmath.pi))*( this.bigmath.log(cableCoaxial.b/cableCoaxial.a) + 1/4 +  x*y);
            let C = (2*this.bigmath.pi*epsilon)/this.bigmath.log(cableCoaxial.b/cableCoaxial.a);
            let z = 1/this.bigmath.pow(cableCoaxial.a, 2) + 1/(this.bigmath.pow(cableCoaxial.c,2) - this.bigmath.pow(cableCoaxial.b,2));
